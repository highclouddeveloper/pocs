...
class MatchingAgent:
    ...
    # NEW: detect "Contact N" sections
    CONTACT_SEC_RE = __import__("re").compile(r"(?i)^\s*contact\s*(\d+)\b")
    ...
    def resolve(
        ...
        # Contact N handling
        m_contact = self.CONTACT_SEC_RE.match((section or "").strip())
        is_contact = bool(m_contact)
        contact_n = int(m_contact.group(1)) if m_contact else None

        # ---- Step 1: strict exact field match ----
        candidates = [r for r in self.rows if r.get("field_norm") == label_strict]
        if not candidates:
            # sometimes authoring introduced slightly different strict norms; try loose-as-strict
        ...
        if candidates:
            ...
            if section_strict:
                if is_contact:
                    # For Contact N: REQUIRE same section; if none, fallback to same field with Index == contact_n
                    same_sec = [r for r in candidates if r.get("section_norm") == section_strict]
                    if same_sec:
                        candidates = same_sec
                    else:
                        # fallback: rows with this field whose Index equals the contact number (or the requested index)
                        candidates = [r for r in candidates if r.get("Index") is not None and int(r["Index"]) in (int(contact_n), int(index))]
                        if not candidates:
                            return None
                else:
                    same_sec = [r for r in candidates if r.get("section_norm") == section_strict]
            ...
            if page is not None:
                same_pg = [r for r in candidates if r.get("Page") is not None and str(r["Page"]).strip() != "" and int(r["Page"]) == int(page)]
                if same_pg:
                    candidates = same_pg

            # Index handling
            if strict_index:
                with_idx = [r for r in candidates if r.get("Index") is not None and str(r.get("Index")).strip() != ""]
                if with_idx:
                    if is_contact and contact_n is not None:
                        # Accept Index == contact_n OR 1 (sheet often uses 1)
                        exact_idx = [r for r in with_idx if int(r["Index"]) in (int(index), int(contact_n), 1)]
                    else:
            ...
            # IMPORTANT: For Contact N do NOT use the single no-index fallback
            if best is None and strict_index and not is_contact:
        ...
        # Section handling in fuzzy path
        if section_loose:
            if is_contact:
                # REQUIRE same section for Contact N
                candidates = [r for r in candidates if r.get("_section_norm_loose") == section_loose]
            else:
                same_sec = [r for r in candidates if r.get("_section_norm_loose") == section_loose]
                if same_sec:
                    candidates = same_sec

        # Index handling again
        if strict_index:
            with_idx = [r for r in candidates if r.get("Index") is not None and str(r.get("Index")).strip() != ""]
            if with_idx:
                if is_contact and contact_n is not None:
        ...
        best = max(
            candidates,
            key=lambda r: (
                1 if (r.get("Index") is not None and str(r.get("Index")).strip() != "" and (
                        (int(r["Index"]) == int(index)) or (is_contact and contact_n is not None and int(r["Index"]) in (int(contact_n), 1))
                )) else 0,
...
class FillingAgent:
    ...
    # Add this helper near the top of fill_tables (next to _row0_texts/_looks_per_column_headers)
    def _table_scope_text(tbl) -> str:
        """
        Try to pull the long 'scope' sentence that lives inside the table itself
        (often row 0 / col 0) — e.g., the 'added to' vs 'removed from' sentence.
        If it's short or empty, return '' so we gracefully fall back.
        """
        try:
            if tbl.rows and tbl.rows[0].cells:
                s = self._first_cell_text(tbl.rows[0].cells[0]).strip()
                s = re.sub(r"[:：﹕꞉˸]\s*$", "", s).strip()
                s = re.sub(r"\s+", " ", s)
                # treat as a meaningful scope only if it's long-ish (likely a paragraph/heading)
                if len(s.split()) >= 6:
                    return s
        except Exception:
            pass
    ...
    def fill_tables(self, dry_run=False):
        ...
        Smart table filling:
          ...
          • Per-column-header tables (e.g. 'Contact 1', 'Contact 2', ...):
            write into the SAME CELL for each column and resolve with the nearest
            active column header ('Contact N') that persists down the column.
        """
        from docx.table import Table
        CONTACT_RE = re.compile(r"(?i)\bcontact\s*(\d+)\b")
        ...
        for tbl in self.doc.tables:
            ...
            # --- Track active section per column (updated as we find "Contact N" headers)
            active_sec = []
            for i, t in enumerate(row0_texts):
                if CONTACT_RE.search(t or ""):
                    active_sec.append(t.strip())
                else:
                    active_sec.append(f"Contact {i+1}")
            ...
            for ri, row in enumerate(tbl.rows):
                ...
                # Detect header row updates (e.g., "Contact 3 | Contact 4")
                for ci in range(len(cells)):
                    hdr = self._first_cell_text(cells[ci]).strip()
                    if not hdr:
                        continue
                    hdr = re.sub(r"[:：﹕꞉˸]\s*$", "", hdr)
                    hdr = re.sub(r"\s+", " ", hdr)
                    if CONTACT_RE.search(hdr):
                        if ci < len(active_sec):
                            active_sec[ci] = hdr
                        else:
                            active_sec.append(hdr)
                ...
                for ci in col_iter:
                    ...
                    if is_percol:
                        sec_hint = active_sec[ci] if ci < len(active_sec) else f"Contact {idx_guess}"
                        m = CONTACT_RE.search(sec_hint)
                        idx_contact = int(m.group(1)) if m else idx_guess

                        if dry_run:
                            print(f"[DRY][percol] col={ci} sec_hint='{sec_hint}' idx_contact={idx_contact} label='{lab}'")
...
def run_agentic_fill(input_docx: str, lookup_path: str, output_docx: str, dry_run: bool = False) -> int:
    """
    Fill-only pipeline. Runs existing passes. If the DOCX has no real checkbox
    glyphs, all checkbox-creation passes are skipped to avoid stray boxes.
    A conservative fallback runs at the end with a glyph-guard.
    """
    try:
    ...
    # --- helpers used in this function only ---
    _CHECKBOX_GLYPH_RE = _re.compile(r"(?:\[\s*[xX✓]?\s*\]|[□☐☑☒])")

    def _doc_has_checkbox_glyphs(d) -> bool:
        # Look anywhere: body paragraphs + every table cell's paragraphs
        for p in d.paragraphs:
            if _CHECKBOX_GLYPH_RE.search(p.text or ""):
                return True
        for t in d.tables:
            for row in t.rows:
                for cell in row.cells:
    ...
    def _is_ph(s: str) -> bool:
        return _is_placeholder_like(s)

    def _toggle_cb(text: str, val: str) -> str:
        return _toggle_checkbox_glyphs_in_text(text, val)

    def _set_par_text(par, txt: str):
        _set_paragraph_text(par, txt)

    def _write_inline(par, v: str):
        _write_value_into_paragraph_after_label(par, v)

    def _iter_paragraph_sequences(d):
        yield d.paragraphs
        for t in d.tables:
            for row in t.rows:
                for cell in row.cells:
    ...
    # Detect if the DOCX truly contains checkbox glyphs
    has_real_checkboxes = _doc_has_checkbox_glyphs(doc)
    print(f"☑️  Real checkbox glyphs present: {has_real_checkboxes}")

    # 3) Match / resolve
    matcher = MatchingAgent(rows)

    # 4) Fill (existing passes)
    ...
    #    Only run checkbox-related passes if the document actually has checkbox glyphs.
    if has_real_checkboxes:
        ...
        print("↷ Skipping checkbox-creation passes (no real glyphs detected).")

    # 2) matrix early
    filler.fill_matrix_tables(dry_run=dry_run)

    # 3) other table strategies
    ...
    # Only run horizontal option group checkbox pass if real glyphs exist
    if has_real_checkboxes:
        filler.fill_checkbox_option_groups(dry_run=dry_run)
    else:
        print("↷ Skipping checkbox option-group pass (no real glyphs detected).")

    # ---------- Conservative last-chance fallback (fill-only, glyph-guarded) ----------
    # Keep only rows with non-empty values
    plan = []
    for r in rows:
        ...
        if not v:
            continue
        try:
            idx = int(r.get("Index", 1) or 1)
        except Exception:
            idx = 1
        is_cb = str(r.get("Choices", "") or "").strip().lower() == "checkbox"
        plan.append({
            "key": _nk(str(r.get("Field", "") or "")),
    ...
    rows_by_key = _dd(list)
    for it in plan:
        rows_by_key[it["key"]].append(it)

    seen_counts = {}

    def _label_match(ptxt: str, key: str) -> bool:
        t = _nk(ptxt)
        if not t:
            return False
        return t == key or t.startswith(key + " ") or t.startswith(key + ":")

    fallback_actions = []
    for pars in _iter_paragraph_sequences(doc):
        i = 0
        while i < len(pars):
            p = pars[i]
            raw_text = p.text or ""
            ...
            for key, targets in rows_by_key.items():
                if not _label_match(raw_text, key):
                    continue

                seen_counts[key] = seen_counts.get(key, 0) + 1
                occ = seen_counts[key]
                target = next((t for t in targets if t["index"] == occ), None)
                if not target:
                    continue

                val = target["val"]
                if target["is_checkbox"]:
                    # GLYPH GUARD x2: only toggle if a real checkbox glyph is in this or next paragraph
                    if _CHECKBOX_GLYPH_RE.search(raw_text):
                        toggled = _toggle_cb(raw_text, val)
                        if toggled != raw_text:
                            fallback_actions.append(f"[fallback] checkbox: '{target['raw']}' idx={occ} -> '{val}'")
                            if not dry_run:
                                _set_par_text(p, toggled)
                    elif i + 1 < len(pars) and _CHECKBOX_GLYPH_RE.search(pars[i + 1].text or ""):
                        nxt = pars[i + 1]
                        nxt_text = nxt.text or ""
                        toggled = _toggle_cb(nxt_text, val)
                        if toggled != nxt_text:
                            fallback_actions.append(f"[fallback] checkbox(next): '{target['raw']}' idx={occ} -> '{val}'")
                            if not dry_run:
                                _set_par_text(nxt, toggled)
                    else:
                        # No glyph nearby → do nothing (prevents stray squares)
                        fallback_actions.append(
                            f"[skip] no-checkbox-glyph near '{target['raw']}' idx={occ}; not creating one"
                        )
                else:
                    wrote = False
                    if i + 1 < len(pars):
                        ...
                        if _is_ph(nxt_text):
                            fallback_actions.append(f"[fallback] next-line: '{target['raw']}' idx={occ} = '{val}'")
                            if not dry_run:
                                _set_par_text(nxt, val)
                            wrote = True
                    if not wrote and val not in (raw_text or ""):
                        fallback_actions.append(f"[fallback] inline: '{target['raw']}' idx={occ} = '{val}'")
                        if not dry_run:
                            _write_inline(p, val)
                break  # one key per paragraph for fallback
...
# ---------------------------
def main():
    ap = argparse.ArgumentParser(description="Agentic DOCX filler (fill-only)")
    ap.add_argument("--input",  required=True, help="Input .docx file to fill")
    ...
    ap.add_argument("--dry-run", action="store_true", help="Analyze and print actions without writing file")
    args = ap.parse_args()
