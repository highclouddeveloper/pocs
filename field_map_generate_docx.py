...
def build_docx_template(input_path: str, out_json: str | None = None) -> dict:
    """
    Build a DOCX template from common subscription/registration forms.

    Detects:
      • Section headings
      • Paragraph label + next 'blank' paragraph (underscores OR mostly whitespace like U+2002/U+2003)
      • Inline underline: 'Label: ________'
      • Checkbox-like lines: '□ Option' or '[ ] Option'
      • 2-col table pairs (left cell label)
      • Simple grid tables (header emits multiple indices)

    Emits field dicts with keys: section, label, label_short, page="", index, placement, type, choices?
    """
    import re
    import json
    import unicodedata
    ...
    def _looks_like_section_title(text: str) -> bool:
        ...
        if len(t) > 180:
            return False
        # treat uppercase-ish or colon-terminated as section
    ...
    # A paragraph that looks like a blank (underscores OR mostly whitespace glyphs)
    UNDERLINE_RUN = re.compile(r"_+")
    # many forms use en/em spaces or figure spaces instead of underscores for lines
    MOSTLY_SPACEY = re.compile(r"^[\s\u2000-\u200B\u00A0.-]{3,}$")  # spaces, thin spaces, nbsp, dots/dashes
    ...
    # ---------- pass 1: detect headings and paragraph fields ----------
    i = 0
    while i < len(blocks):
        b = blocks[i]
        ...
        if isinstance(b, Paragraph):
            ...
            if txt:
                ...
                # inline underline on same line (Label: ______)
                ...
                if m:
                    ...
                    if lab:
                        fields.append({
                            ...
                            "page": "",
                            "index": 1,
                            "placement": "docx_underline_inline",
                            "type": "text",
                        })
                    i += 1
                    continue
                ...
                if m:
                    ...
                    if lab and 1 <= len(lab.split()) <= 10:
                        fields.append({
                            ...
                            "page": "",
                            "index": 1,
                            "placement": "docx_checkbox_line",
                            "type": "checkbox",
                            "choices": [lab],  # downstream marks as checkbox
                        })
                    i += 1
                    continue

                # label + next-line blank (either underscores or mostly spacey glyphs)
                # Example in your form:
                #   "Last name"
                #   "     "   (U+2002/2003 etc.)  ← treat as a blank line
                if i + 1 < len(blocks) and isinstance(blocks[i + 1], Paragraph):
                    nxt_text = (blocks[i + 1].text or "").strip()
                    is_blank_line = bool(UNDERLINE_RUN.search(nxt_text) or MOSTLY_SPACEY.match(nxt_text))
                    if is_blank_line:
                        ...
                        if lab and len(lab) <= 80:
                            fields.append({
                                ...
                                "page": "",
                                "index": 1,
                                "placement": "docx_underline_nextline",
                                "type": "text",
                            })
                        i += 2
                        continue
        ...
        if isinstance(b, Table):
            ...
            if ncols >= 2:
                # If first row looks like a header row and below looks mostly blanks, treat as grid
                header_cells = b.rows[0].cells
                headers = [_strip_colon_like(_norm_space(_cell_text(c))) for c in header_cells]
                header_nonempty = [h for h in headers if h]

                # Heuristic to detect grid with several data rows that are mostly empty/boxes
                def _cell_blankish(c):
                    t = _cell_text(c)
                    if not t:
                        return True
                    t2 = t.strip()
                    return bool(UNDERLINE_RUN.search(t2) or MOSTLY_SPACEY.match(t2)) or (len(t2) <= 2)

                looks_grid = False
                if nrows >= 3 and len(header_nonempty) >= 2:
                    blank_row_like = sum(
                        1 for r in b.rows[1:]
                        if sum(1 for c in r.cells if _cell_blankish(c)) >= max(2, int(0.6 * ncols))
                    )
                    if blank_row_like >= max(1, (nrows - 1) // 2):
                        looks_grid = True

                if looks_grid:
                    # emit each header as a field (Index=1; exporter can expand more indices if desired)
                    for htxt in headers:
                        if not htxt:
                            continue
                        fields.append({
                    ...
                    # treat each row as label/value pair(s) using the first column as label
                    for r in b.rows:
